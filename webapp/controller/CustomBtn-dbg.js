...
sap.ui.define([
	...
	Button.prototype.ontouchend = function (oEvent) {
		...
		if (this._bRenderActive) {
			delete this._bRenderActive;
			if (oEvent.originalEvent && oEvent.originalEvent.type in {
					mouseup: 1,
					touchend: 1
				}) {
				this.ontap(oEvent);
		...
		if (!sap.ui.Device.browser.msie) {
			...
			if (this._buttonPressed === 0 && ((this._sStartingTagId === "-BDI-content" && (this._sEndingTagId === "-content" || this._sEndingTagId ===
					"-inner" || this._sEndingTagId === "-img")) || (this._sStartingTagId === "-content" && (this._sEndingTagId === "-inner" || this._sEndingTagId ===
					"-img")) || (this._sStartingTagId === "-img" && this._sEndingTagId !== "-img"))) {
				this.ontap(oEvent);
			}
		}
	...
	Button.prototype.onkeydown = function (oEvent) {
		...
		if (oEvent.which === KeyCodes.SPACE || oEvent.which === KeyCodes.ENTER || oEvent.which === KeyCodes.ESCAPE || oEvent.which ===
			KeyCodes.SHIFT) {
